...
#pragma omp for schedule(dynamic)
    for (std::uint16_t j = 0; j < static_cast<std::uint16_t>(num_views); ++j) {
      ...
      int idx = 0;
      std::vector<size_t> face_ids(faces.size()/3,false);
...
//#pragma omp for schedule(dynamic)
    for (std::uint16_t j = 0; j < static_cast<std::uint16_t>(num_views); ++j) {
      open3d::geometry::TriangleMesh mesh_clone = o3d_mesh;
      ...
      std::cout<<"view: "<<j<<std::endl;
      int idx = 0;
      std::vector<bool> face_ids(faces.size()/3,true);
      for (std::size_t i = 0; i < faces.size(); i += 3) {
        std::size_t face_id = i / 3;
        ...
        projected_face_view_infos.push_back(pair);
        idx++;
        face_ids[face_id] = false;
      }
      std::cout<<"visibility percent :"<<(double)std::count(face_ids.begin(),face_ids.end(),false)/face_ids.size()<<std::endl;
      //texture_view->world_to_cam
      mesh_clone.RemoveTrianglesByMask(face_ids);
      mesh_clone.Transform(texture_view->extrinsic_);
      open3d::io::WriteTriangleMesh("mesh-"+std::to_string(j)+".ply",mesh_clone);
      //texture_view->release_image();
      //texture_view->release_validity_mask();
      if (settings.data_term == DATA_TERM_GMI) {
        //texture_view->release_gradient_magnitude();
